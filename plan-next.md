...
1. Revisit the key when a rename touches a candidate for it. Guessing pairs candidates by name, or by a hint that identified one across a rename, so a renamed key column is invisible to it unless a paired `--key` component names both sides: the demo shows `customer_id`/`id` falling through to `amount` and reporting the changed row as a drop and an add. Inference does find the rename, but `resolve_key` runs before `match_rows` and the identity arrives too late to match rows with. Two ways out are worth weighing. Either resolve the key again once rename inference has identified a column that was a candidate, which means deciding whether a second pass stays deterministic, whether it can loop, and which result wins when the two passes disagree. Or infer key renames before row matching, pairing rows by position for the purposes of that inference alone — this needs the row-number machinery from the previous item, plus a rule for how far positional evidence may be trusted when the two files differ in row count or order, since that is exactly the case where it is worth least.
