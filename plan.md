...
- [x] **Read a printed line back whatever detail it carries.** A hint's claim is its first argument; a field, or one of the flags the format writes, after it is detail the format prints about the operation, not part of the claim, and is ignored. This is what keeps `col_rename(amount -> total, basis: exact)` an instruction, and it makes `col_edit(value, values)` one for the first time.
...
* `src/hint.rs`: `Hints` loses its `Default`, `validate_edits` takes the map, and the parser ignores an argument after the first that is a field or a known flag.
...
* `src/human.rs`: `basis` on every `col_rename()` line.
...
The fix is to say what the parser has always assumed and never enforced: **a hint's claim is its first argument, and anything after it is detail.** A later argument must be a field or one of the flags the format writes, and is ignored. That accepts `basis: exact`, `values`, and `type: Int32 -> Int64` — every suffix the format writes — and refuses `col_rename(a -> b, c -> d)`, whose second argument is shaped like a second claim and is much likelier to be a user meaning something else. Ignoring rather than checking is right because none of that detail is the user's to assert: what a hint contributes is the identity, and supplying `basis: exact` does not make the basis exact — it makes it `hinted`, which is what the hint is.

Detail is recognized two different ways, and deliberately. A field carries the grammar's colon, which cannot be mistaken for a claim, so any field is detail and the parser does not read the field names — that list belongs to the renderer. A flag has no such marker, so the flags are named, and `values` is the only bare word the format writes on a line of a hint kind. Admitting bare words in general would be the cheaper rule and the wrong one: it would make `col_edit(price, cost)`, a user naming two columns, quietly mean `col_edit(price)`.
...
* `src/hint.rs`: each suffix the format writes — `basis: exact`, `values`, `type: Int32 -> Int64` — parsing to the same claim as the bare line; a second pair-shaped argument still malformed; and a name that legitimately contains a comma still needing its quotes.
...
* The rename round-trip test carries the printed line back with its field and asserts identity preserved and basis changed, as above; a second round-trip feeds back a printed `col_edit(value, values)`.
...
* any printed line whose operation is one of the four column-hint kinds is accepted as a hint of that kind, the detail after the first argument being ignored, and the round-trip test says that identity survives and basis does not;
