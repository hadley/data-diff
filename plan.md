...
The remaining tests currently read the diff through `serde_json::to_value` and asserts against `json!` literals with one-based coordinates. Each assertion becomes a direct comparison against the matching `Diff` field, with expected values built through the public constructors:
