...
- [x] **Document the absence of a row minimum.** The design's 20-row minimum is deliberately not implemented, so the reason has to live where the thresholds are applied: a comment beside the agreement predicates in `src/agreement.rs` recording that the minimum was considered and dropped, what $\kappa$ does and does not cover, and the implicit eleven-row floor that $p_o > 0.9$ imposes on approximate renames but not on swaps.
...
* neither new stage imposes a row minimum, `design.md` no longer asks for one, and the reasoning is recorded beside the predicates in `src/agreement.rs` rather than only in this plan;
