...
Three stages have a worst case that is not linear in the input — rename inference examines candidate pairs, swap inference measures crossings, and edit summarization runs a maximum matching — and each runs under a counted budget so the whole pipeline is predictably bounded. Every budget is a count of deterministic work units — rows sampled, first-time pair examinations, changed cells admitted to the exact cover — never elapsed time, which would make the output depend on the machine. The units are consumed in a deterministic order, so the set of examinations that fit under a budget is a pure function of the input and options, and repeated runs stay byte-identical whether the budgets bind or not. An examination is charged once: every answer about a pair is memoized, and a repeated question costs nothing.

The four budgets live on `DiffOptions` with benchmark-tuned defaults chosen so that ordinary inputs never bind them; the CLI does not yet expose them, that being a UI decision the queue defers. `agreement_rows` (4096) caps the matched rows approximate rename and swap inference measure over; `rename_pairs` (2048) counts rename inference's value-level pair examinations across both of its stages; `swap_pairs` (2048) counts swap inference's crossing measurements; and `summary_cells` (10 000) caps the changed cells the summary's exact minimum cover is computed over. The tuning rule is measured on `benches/pipeline.rs` (2026-08-04) rather than felt: over a grid of rows {1 000, 100 000, 1 000 000} by columns {10, 100, 1 000} capped at 10⁷ cells, each bounded stage completes within twice the same-sized identical run — the pipeline's unavoidable linear pass — on every adversarial scenario, and no non-adversarial scenario reports an incomplete stage. The pair budgets landed a halving below the row sample because a unit touches up to a sample's worth of rows, so at the grid's smallest tables a budget an order larger buys the adversaries more time than the whole linear pass it is measured against. `agreement_rows` has a heuristic floor as well as a cost ceiling, recorded here so it is not tuned away: observed agreement is a proportion whose sampling error is at most $1/(2\sqrt{n})$ — under 0.8% at 4096 rows against a 90% bar, where 1000 rows would leave ~1.6% — while $\kappa$'s uncertainty, depending on both estimated frequency distributions, is covered only indicatively, and the bottom-$k$-by-hash selection stands in for a uniform random sample only insofar as the hash spreads canonical key bytes uniformly. Benchmarks may raise the sample for speed's sake but should not push it much lower.
