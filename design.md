...
Two rules keep it regular, and both are load-bearing rather than stylistic. Every field carries a colon, so a keyword is never mistakable for a bare value. Field names are drawn from a fixed set — `basis`, `changed`, `overlap`, `type` — rather than varying with what they describe, so `col_key(["id"], basis: guessed, overlap: 0.67)` rather than putting the basis in the name position.
...
Every argument but the first is a field, in every line the format writes. A bare word in that position would be indistinguishable from a name — `col_edit(values)` is a column called `values`, and nothing should have to be known about the vocabulary to see that — so the fact a column's values changed is written `changed: values` rather than as a bare flag. It carries no evidence of its own, unlike `type: Int32 -> Int64`, and it is needed regardless: without it an edit that changed both type and values reads exactly like one that changed only its type.
...
`col_edit()` records independent `type_changed` and `values_changed` aspects for an identified column, written `type: old -> new` and `changed: values`. Schemas and cells remain the underlying evidence. A type-only change has no changed cells and appears only in the schema display; source-type changes never manufacture cells. Rename and edit are independent facts.
