...
The work outside the budgets is deliberately unbudgeted, its cost linear or nearly so and recorded here (2026-08-05): input canonicalization, key validation, guessing, and row matching are linear passes over columns and rows, with each side's key tuples held flat and sequence-hashed exactly once, at resolution — index bucketing, row matching, and sample selection all read those stored digests rather than re-hashing; projection construction behind inference is lazy and per-piece — a sampled measurement takes the sampled rows out of the column and canonicalizes only those, and the full projection, its digest, and its frequency counts are each built when a full-row question first asks, at most once per (column, plan), the plans a column can take part in being bounded by the comparison matrix's own row for its family, so full projections cost linear in cells times a matrix-bounded constant and sampled ones cost only the sample; the exact stage's digest join is one hash lookup per (drop, partner plan); swap inference's rewritten filter is one sampled measurement per eligible identity; cell comparison is linear in identities times matched rows and is never sampled or bounded, the complete cell-level diff being a retained invariant; and the ordering analysis is linearithmic. Value hashing streams its fixed byte encoding into the hasher without allocating per value, and the internal hash maps on these paths use cheap deterministic hashers rather than std's SipHash — safe because every lookup confirms real equality and no such map's iteration order reaches output. Bounding key guessing and uniqueness checking was considered and deferred on the benchmark evidence, which shows them dwarfed by cell comparison.
