...
The work outside the budgets is deliberately unbudgeted, its cost linear or nearly so and recorded here (2026-08-04): input canonicalization, key validation, guessing, and row matching are linear passes over columns and rows; projection construction behind inference — canonicalizing and projecting one column under one comparison plan — is built at most once per (column, plan) and the plans a column can take part in are bounded by the comparison matrix's own row for its family, so the whole projection pass is linear in cells times a matrix-bounded constant; the exact stage's digest join is one hash lookup per (drop, partner plan); swap inference's rewritten filter is one measurement per eligible identity; cell comparison is linear in identities times matched rows and is never sampled or bounded, the complete cell-level diff being a retained invariant; and the ordering analysis is linearithmic. Bounding key guessing and uniqueness checking was considered and deferred on the benchmark evidence, which shows them dwarfed by cell comparison.
